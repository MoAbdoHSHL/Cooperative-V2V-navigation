...
import serial
import paho.mqtt.client as mqtt  # Added MQTT import

os.environ['OMP_NUM_THREADS'] = '1'
os.environ['OPENCV_VIDEOIO_DEBUG'] = '0'

# MQTT Configuration (Add this at the top with other constants)
BROKER_IP = "172.20.10.13"  # IP of Pi #2 (broker)
TOPIC = "V2V"

# Initialize MQTT client (Add this after other configurations)
...
running = True
last_mqtt_sent_time = 0  # Track when last MQTT message was sent
mqtt_cooldown = 3  # Seconds between MQTT messages

# ----------------- MQTT FUNCTIONS ------------------
def send_mqtt_message(message):
    global last_mqtt_sent_time
    current_time = time.time()
    
    # Only send if cooldown period has passed
    if current_time - last_mqtt_sent_time >= mqtt_cooldown:
        try:
            mqtt_client.publish(TOPIC, message)
            print(f"MQTT message sent: {message}")
            last_mqtt_sent_time = current_time
...
def lidar_loop():
    global scan_data, current_angle, stop_requested, running, object_detected
    ...
    while running:
        try:
            ...
            if 0 < dist <= 30:
                ...
                send_mqtt_message("LIDAR_OBSTACLE")  # Send MQTT message
            else:
...
def process_frame(frame, model):
    ...
    for box in result.boxes:
        ...
        if conf > 0.5:  # Only consider confident detections
            ...
            # Send MQTT message with detected object type
            send_mqtt_message(f"OBJECT_{class_names[cls_id].upper()}")
...
def main():
    ...
    try:
        ...
        ser.close()
        mqtt_client.disconnect()  # Clean up MQTT connection
